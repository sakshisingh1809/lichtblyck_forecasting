...
def _pfl_nestedtree_taxis0(
    pfl_dict: Dict[str, PfLine], cols: Iterable[str], depth: int
) -> Iterable[str]:
    ...
    for c, (name, pfl) in enumerate(pfl_dict.items()):
        ...
        for txtline in _pfl_flatdatablock_taxis0(pfl, cols, 5):
            out.append(tree["10"] + tree["11"] + colorama.Style.RESET_ALL + txtline)
        # Children.
        for txtline in _pfl_nestedtree_taxis0(pfl.children, cols, depth + 1):
...
def pfl_as_string(pfl: PfLine, flatten: bool, num_of_ts: int, color: bool) -> str:
    ...
    if flatten:
        ...
        lines.extend(_pfl_nestedtree_taxis0({"(this pfline)": pfl}, cols, 0))
    txt = "\n".join(lines)
...
class PfLineText:
    ...
    def print(
        self: PfLine, flatten: bool = False, num_of_ts: int = 6, color: bool = True
    ) -> None:
        """Treeview of the portfolio file.

        Parameters
        ----------
        flatten : bool, optional (default: False)
            if True, show only the top-level (aggregated) information.
        num_of_ts : int, optional (default: 6)
