...
def aggdata():
    ...
    for ts, val, dur in zip(source.index, source.values, source.duration):
    ...
    agg_data["15T"] = {
        ...
        "durations": [[d] for d in source.duration],
        "index": source.index,
    }
    return agg_data
    ...
    for coll, combis in [(summed, sumcombis), (avged, avgcombis)]:
        for freq1, s1 in coll.items():
            for freq2, s2 in coll.items():
                if functions.freq_up_or_down(freq1, freq2) > 0:
                    continue
...
def test_changefreq_sum(fr1, fr2, descr):
    testfr = functions.changefreq_sum(fr1, fr2.index.freq)
...
def test_changefreq_avg(fr1, fr2, descr):
    testfr = functions.changefreq_avg(fr1, fr2.index.freq)
    if isinstance(fr1, Series):
        pd.testing.assert_series_equal(testfr, fr2)
    else:
        pd.testing.assert_frame_equal(testfr, fr2)
