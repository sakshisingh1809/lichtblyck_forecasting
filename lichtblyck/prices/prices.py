...
def spot() -> pd.Series:
    """Return spot price timeseries."""
    data = pd.read_excel(SPOTPRICEFILE, sheet_name='spot_static', header=0, 
                         usecols='B:AC', index_col=-1)
    data = data.drop(data.columns[-2:], axis=1)
    dls_values = data['DLS'].dropna() #prices for second 02:00-03:00 hour on last Sun in Oct.
    ...
    for ts in [i + pd.offsets.LastWeekOfMonth(weekday=6) + pd.Timedelta(hours=2)
...
def futures(prod:str='y', earliest_trading=None, earliest_delivery=None) -> pd.DataFrame:
    ...
    return df[['ts_right_deliv', 'trade_before_deliv', 
...
def frontmonth():
    ...
    """
    # Get values...
    b = pd.read_excel(FRONTMONTHPRICEFILE, sheet_name='futures_static', usecols=[4,8,9],
                         header=0, names=['p_base', 'ts_left_trade', 'ts_left_deliv'])
    p = pd.read_excel(FRONTMONTHPRICEFILE, sheet_name='futures_static', usecols=[16,20,21],
                         header=0, names=['p_peak', 'ts_left_trade', 'ts_left_deliv'])    
    ...
    return df[['ts_left_deliv', 'ts_right_deliv', 'trade_before_deliv', 
               'p_base', 'p_peak', 'p_offpeak', 
               'basehours', 'peakhours', 'offpeakhours']]
...
def _w_hedge(w:Union[pd.Series,Iterable], p:Union[pd.Series,Iterable]) -> pd.Series:
    ...
    """
    grouped = pd.DataFrame({'w':w, 'p':p}).groupby(is_peak_hour)\
        .apply(lambda df: tools.wavg(df['w'], df['p']))
    return pd.Series({'w_peak': grouped[True], 'w_offpeak': grouped[False]})
def w_hedge(w:Union[pd.Series,Iterable], p:Union[pd.Series,Iterable],
    ...
    if freq not in ('MS', 'QS', 'YS'):
        raise ValueError("Argument 'freq' must be in {'MS', 'QS', 'YS'}.")
    return pd.DataFrame({'w':w,'p':p}).groupby(pd.Grouper(freq=freq))\
        .apply(lambda df: _w_hedge(df['w'], df['p']))
def w_hedge_long(w:Union[pd.Series,Iterable], p:Union[pd.Series,Iterable],
                 freq:str=None) -> pd.Series:
    """
    ...
    if freq is None:
        ...
        raise ValueError("Argument 'freq' must be in {'MS', 'QS', 'YS'}.")
    apply_function = lambda df: pd.Series([tools.wavg(df['w'], df['p'])]*len(df), df.index)
    s = pd.DataFrame({'w':w, 'p':p}).groupby(group_function)\
        .apply(apply_function).droplevel(0).rename('w_hedge')
