...
# Get the tlp profile... 
t2l = 0.9*lb.tlp.standardized_tmpr_loadprofile('bayernwerk_nsp') \
    + 0.1*lb.tlp.standardized_tmpr_loadprofile('bayernwerk_wp')   #2=nsp, 3=wp
# ...and plot to verify.
wide = t2l.unstack('t') #or: wide = profile.reset_index().pivot(index='time_left_local', columns='tmpr', values='std_tmpr_lp')
...
q = [lb.tlp.tmpr2load(t2l, tmpr['t_germany'] + dt, 0.79e6).mean() * 8760 / 1e6 for dt in delta_t] #in /1e6 to make TWh
