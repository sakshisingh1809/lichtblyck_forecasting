...
prog = pd.read_excel('scripts/2020_10 Beschaffungskomponente Power B2C/20201005_Input für die Beschaffungskomponente.xlsx',
                     header=0, skiprows=1, index_col=0, usecols=[0,1,2,3,4],
...
def w_offtake(churn_2021_as_frac_of_100pct:float):
    ...
    churnpath = churnpath_of_expvolume * churn_2021_as_frac_of_100pct / churnfrac_of_expvolume
...
ax[1].plot(prog[['w_100pct', 'w_exp', 'w_certain']].resample('D').mean().div(ref, axis=0), color='r')


# %% PRICES

# Get prices with correct expectation value (=average).
p_sims = pd.read_csv('scripts/2020_10 Beschaffungskomponente Ludwig/MC_NL_HOURLY_PWR.csv')
