...
weights = weights["power"] / weights["power"].sum()
...
t_permonth.loc["std"] = t_permonth.loc[:"mean"].std()
...
offtake = {}
for pf, details in {
    "rh": {"source": 2, "spec": 492831},
    "hp": {"source": 3, "spec": 46970},
}.items():
    # Temperature -> offtake.
    ...
    for delta_t in tqdm(range(-10, 11)):
        ...
        offtake[(pf, Q_(delta_t, "degC"))] = offtake_percalmonth
offtake = pd.DataFrame(offtake)

# Result 1: offtake and sensitivity at actual temperatures, per portfolio.
...
result2 = offtake.T.groupby(level=1).sum().transform(getresult2).T
result2.to_csv("p2h_offtakescalingfactors_per_calmonth_and_degC.csv")
#
