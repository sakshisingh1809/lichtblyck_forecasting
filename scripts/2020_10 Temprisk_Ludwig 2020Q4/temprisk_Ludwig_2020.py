...
df = pd.read_excel("scripts/2020_10 Temprisk_Ludwig/Zeitreihenbericht.xlsx",
                   header=1)
df = lb.tools.set_ts_index(df, "ZEITSTEMPEL", "right")
df.columns = pd.MultiIndex.from_tuples((
    ('pfc', 'pfc', 'p'),
    ('rh', 'current', 'w'), ('hp', 'current', 'w'), ('rest', 'current', 'w'),
    ('rh', 'certain', 'w'), ('hp', 'certain', 'w'), ('rest', 'certain', 'w'),
    ('rh', 'fwd', 'w'), ('hp', 'fwd', 'w'), ('rest', 'fwd', 'w'),
    ('rh', 'fwd', 'r'), ('hp', 'fwd', 'r'), ('rest', 'fwd', 'r')))
...
df[('rest', 'exp', 'w')] = df.rest.current.w + (df.rest.certain.w - df.rest.current.w) * factor


#%% Find temperature load profiles, that fit to the expected, warm, and cold yearly offtake

# Expected temperatures.
ti = pd.date_range('2021-01-01', '2022-01-01', freq='D', tz='Europe/Berlin', closed='left')
...
t2l = pd.DataFrame({'rh': lb.tlp.standardized_tmpr_loadprofile(2),
                    'hp': lb.tlp.standardized_tmpr_loadprofile(3)})
spec = pd.Series({'rh': 612620, 'hp': 53388}) #to find spec: spec = (1-churn_exp) * df.hp.current.w.q.sum() / exp.hp.w.q.sum()
...
offtake = pd.DataFrame({('norm', 'w'): offtake_ludwig_heating(0),
                        ('cold', 'w'): offtake_ludwig_heating(delta.cold.t),
                        ('warm', 'w'): offtake_ludwig_heating(delta.warm.t)}) 
assert(np.isclose(offtake.norm.q.sum(), -(1-churn_exp)*(df.rh.current.q.sum() + df.hp.current.q.sum())))#check
delta = pd.concat([delta, pd.DataFrame(
    {'warm': (offtake.warm.q - offtake.norm.q).sum(),
     'cold': (offtake.cold.q - offtake.norm.q).sum()}, index=['q'])])
