"""Update the long-term temprisk from current prices and reference values. (per month)"""
...
ref = lb.portfolios.pfstate("power", pfname, year)

# Current situation based on current portfolio mixprices prices.
...
def pricecurve_sim(
    ...
    """Returns function to calculate price curve at certain quantile. vola_peak and 
    vola_offpeak are given as fraction_per_calenderyear."""
...
def offtakecurve_sim(current_offtake: lb.PfLine, scale_factors: float) -> Callable:
    ...
    scale_factors[0] = 1e-5  # no change in Jan; over.
    scale_factors[1] = 1e-5  # no change in Feb; over.
    scale_factors[2] = 1e-5  # no change in Mar; almost over.
    ...
    def offtakecurve() -> lb.PfLine:
        ...
        factors = lb.changefreq_avg(
            pd.Series(
                ...
                pd.date_range(year, freq="MS", periods=12, tz="Europe/Berlin"),
            ),
            "15T",
        )
...
df_m_premium.tz_localize(None).pint.dequantify().to_excel(f"lttr_{pfname}_monthly.xlsx")
df_q_premium.tz_localize(None).pint.dequantify().to_excel(
    f"lttr_{pfname}_quarterly.xlsx"
)
