...
avgs = [
    (
        val,
        val2 := sum(val2 * dt for val2, tt2 in zip(y2, t2) if tl <= tt2 < tr)
        / (tr - tl),
        val2 - val,
    )
...
def smooth_at_monthboundary(s_in):
    ...
    boundaries = (
        ...
        .set_index(0)
        .rename({1: "Δf", 2: "Δdf"}, axis=1)
        .rename_axis("ts", axis=0)
    )
    ...
    for ts, s in s_in.groupby(pd.Grouper(freq="MS")):
        t = (s.index - s.index[0]) / (
            (s.index[-1] - s.index[0]) * (len(s) + 1) / len(s)
        )  # [0..1)
        try:
        ...
        coeff = smoothing_coefficients(
            wanted["f_l"], wanted["df_l"] * 30, wanted["f_r"], wanted["df_r"] * 30
        )
...
for df in p_fwd["p_base"].rolling(w + 1):
    ...
    polynobound = polynobound.append(
        s[(s.index >= df.index[i]) & (s.index < df.index[i + 1])]
    )
...
for ax in axes.flatten():
    if ax.lines:
        ax.hlines(
            p_fwd["p_base"], p_fwd.index, p_fwd.index + p_fwd.index.freq, "k", zorder=10
        )
