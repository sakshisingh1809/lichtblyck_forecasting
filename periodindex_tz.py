...
distance_h = distance.resample('15T').asfreq().fillna(0)\
    .groupby(pd.Grouper(freq=distance.index.freq)).transform(np.mean)
#   4: downsample a mean quantity
avspeed2 = avspeed_h.resample('M').mean()

dist_notz = speed_notz.resample('D').                       
                       
                       , 'dist': [48, 69, 96]},
                           index=dti)
df_dti_notz.resample('H').ffill()

distance = pd.Series([366], pd.PeriodIndex(['2020'], freq='Y'))
dist_m = distance.resample('M').asfreq().fillna(0).groupby(pd.Grouper(freq=distance.index.freq)).transform(np.mean)
...
df_ts_minute = pd.DataFrame({'speedo':speedometer(hour(idx_ts_minute)), 'odo':odometer(hour(idx_ts_minute))}, index=idx_ts_minute)
# Daily values.
idx_ts = pd.date_range('2020-01-01 00:00', '2020-04-10', freq='D', closed='left')
df_ts = pd.DataFrame({'speedo':speedometer(hour(idx_ts)), 'odo':odometer(hour(idx_ts))}, index=idx_ts)
# Plot.
fig, axes = plt.subplots(1, 2, sharex=True, figsize=(10, 5))
#   continuous
...
distance = lambda pe: odometer(hour((pe+1).start_time)) - odometer(hour(pe.start_time))
...
df_pe = pd.DataFrame({'avspeed': avspeed(idx_pe), 'dist': distance(idx_pe)}, index=idx_pe)
# Helper df for plot.
df = df_pe.copy()
...
axes[1].plot(df_ts_minute['odo'], f'{COL2}--', alpha=0.35, lw=1)
#   discrete
axes[0].hlines(df_pe['avspeed'], df_pe.index.start_time, (df_pe.index+1).start_time, COL1)
axes[1].scatter(df.index.start_time, df['odo_start'], c=COL2, s=4)
axes[1].hlines(df['odo_start'], fracTs(idx_pe-1, 0.05), fracTs(idx_pe, 0.95), COL2, linewidth=1, alpha=0.125)
#   titles, labels and limits
axes[0].title.set_text("Speedometer")
axes[1].title.set_text("Odometer")
axes[0].yaxis.label.set_text("velocity [km/h]")
...
for x, y, dy in zip(df['midTs'], df['odo_start'], df['dist']):
    axes[1].annotate("", xytext=(x, y), xy=(x, y+dy), arrowprops={'color':COL2, 'lw': 1, 'arrowstyle':'-|>'})
#   highlights
i = 2
pe, ts, avspeed, odo_start, dist = df.index[i], df['midTs'][i], df['avspeed'][i], df['odo_start'][i], df['dist'][i]
...
axes[0].annotate(f'Data at row index {i}:\n\nDuring {pe}\n(from {pe.start_time}\nuntil {(pe+1).start_time})\n\nAverage velocity was {avspeed:.2f} km/h',
                 xy=((pe+1).start_time, avspeed), xycoords='data',
                 xytext=(0.4, 0.95), textcoords='axes fraction',
                 arrowprops={'fc':'black', 'alpha':0.6, 'shrink':0.15},
                 horizontalalignment='left', verticalalignment='top')
axes[1].annotate(f'Data at row index {i}:\n\nDuring {pe}\n(from {pe.start_time}\nuntil {(pe+1).start_time})\n\nDistance (i.e., increase in\nodometer value) was {dist:.2f} km',
                 xy=(ts, odo_start+dist/2), xycoords='data',
                 xytext=(0.45, 0.35), textcoords='axes fraction',
                 arrowprops={'fc':'black', 'alpha':0.6, 'shrink':0.15},
                 horizontalalignment='left', verticalalignment='top')

#%% Calculating additional values
df_pe['dur'] = ((df_pe.index+1).start_time - df_pe.index.start_time).total_seconds()/3600
...
i = 2
pe, ts, avspeed, odo_start, dist = df.index[i], df['midTs'][i], df['avspeed'][i], df['odo_start'][i], df['dist'][i]
ax.axvspan(pe.start_time, (pe+1).start_time, ymax=1, ymin=avspeed/1.5, alpha=0.2, color='grey')
ax.axvspan(pe.start_time, (pe+1).start_time, ymin=0, ymax=avspeed/1.5, alpha=0.4, color=COL2)
ax.annotate(f'Data at row index {i}:\n\nBy multiplying\n· average velocity = {avspeed:.2f} km/h\nwith\n· duration = 24 h,\nwe obtain\n· distance = {dist:.2f} km\nas the area under curve.',
            xy=((pe+1).start_time, avspeed*0.85), xycoords='data',
...
ax.hlines(df['odo_start'], fracTs(idx_pe-1, 0.05), fracTs(idx_pe, 0.95), COL2, linewidth=1, alpha=0.125)
#   titles, labels and limits
...
fig.tight_layout()
for x, y, dy in zip(df['midTs'], df['odo_start'], df['dist']):
    ax.annotate("", xytext=(x, y), xy=(x, y+dy), arrowprops={'color':COL2, 'lw': 1, 'arrowstyle':'-|>'})
#   highlights
i = 2
pe, ts, avspeed, odo_start, dist = df.index[i], df['midTs'][i], df['avspeed'][i], df['odo_start'][i], df['dist'][i]
ax.axvspan(pe.start_time, (pe+1).start_time, alpha=0.2, color='grey')
ax.plot((pe.start_time, (pe+1).start_time), (odo_start, odo_start+dist), alpha=1, color=COL1, lw=2)
ax.annotate(f'Data at row index {i}:\n\nBy dividing\n· distance = {dist:.2f} km\nby\n· duration = 24 h,\nwe obtain\n· average velocity = {avspeed:.2f} km/h\nas the slope of the curve.',
            xy=((pe+1).start_time, odo_start+dist/2), xycoords='data',
